...
const form = () => {
  ...
  useEffect(() => {
    if (query.id) {
      axios.get("/api/atestados/" + query.id).then((resultado) => {
        console.log(resultado.data);
        const atestado = resultado.data;
        for (let atributo in atestado) {
          setValue(atributo, atestado[atributo]);
        }
      });
    }
  }, [query.id]);
  ...
  function salvar(dados) {
    axios.put("/api/atestados/" + query.id, dados);
    push("/atestados");
  }
  ...
  return (
    <>
      <div className={styles.fundo}>
        <Pagina titulo="Alteração de Atestado">
          <Form>
